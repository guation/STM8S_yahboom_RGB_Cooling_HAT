...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
//__IO uint8_t Slave_Buffer_Rx[256];
//__IO uint8_t Tx_Idx = 0, Rx_Idx = 0;
__IO uint8_t reg_Idx = 0;
  ...
  */
  ...
  switch (Event)
  {
    ...
    case I2C_EVENT_SLAVE_RECEIVER_ADDRESS_MATCHED:
      ...
      /* Check on EV2*/
    case (0x0050)://树莓派发送c模式的时候会触发这个标签 不检测的话会导致循环触发中断引起死机
    case I2C_EVENT_SLAVE_BYTE_RECEIVED:
      if(addr){
        addr=0;
        reg_Idx=I2C_ReceiveData();
      }else{
        //Slave_Buffer_Rx[Rx_Idx++] = I2C_ReceiveData();
        set_task(reg_Idx++, I2C_ReceiveData());
      }
      break;
